...
def get_datahub_credentials():
    """Retrieve and parse the environment variable which contains a path to a JSON file."""
    data_hub_creds_path = os.environ.get('DATA_HUB_APPLICATION_CREDENTIALS')

    if not data_hub_creds_path:
        raise ValueError("Environment variable 'DATA_HUB_APPLICATION_CREDENTIALS' not found.")

    if not os.path.isfile(data_hub_creds_path):
        raise ValueError(f"'{data_hub_creds_path}' is not a valid file path.")

    with open(data_hub_creds_path, 'r') as file:
        data_hub_creds = json.load(file)

    datahub_url = data_hub_creds.get('datahub_url')
    datahub_token = data_hub_creds.get('datahub_token')

    if not datahub_url or not datahub_token:
        raise ValueError("Expected keys 'datahub_url' and 'datahub_token' not found in the JSON content.")

    return datahub_url, datahub_token
